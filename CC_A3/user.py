...
def get_all_users():
    ...
    get_user=res.post('http://localhost:8080/api/v1/db/read',json=user)
    return get_user
#----------TASK 1:-------
...
def create_user():
    ...
    if not(x):
        return {},400
    s=res.post("http://localhost:8080/api/v1/db/write",json=new_user)
...
def count_http_request():
    ...
    new_user={"tableName":tableName,"func_Name":func_Name}
    # return "hell"
    s=res.post("http://localhost:8080/api/v1/db/read",json=new_user)
    # l=[]
    # l.append(s["count"])    
    return s
#5 RESET COUNT TO 0
